...
class Parser
{
    ...
    public function parse($value, $flags = 0)
    {
        ...
        if (\Symfony\Component\Yaml\Yaml::PARSE_KEYS_AS_STRINGS & $flags) {
    ...
    private function doParse($value, $flags)
    {
        ...
        do {
            ...
            \Symfony\Component\Yaml\Inline::initialize($flags, $this->getRealCurrentLineNb(), $this->filename);
            ...
            if (self::preg_match('#^\\-((?P<leadspaces>\\s+)(?P<value>.+))?$#u', \rtrim($this->currentLine), $values)) {
                ...
                if (!isset($values['value']) || '' == \trim($values['value'], ' ') || 0 === \strpos(\ltrim($values['value'], ' '), '#')) {
                    ...
                    if (isset($values['leadspaces']) && ('!' === $values['value'][0] || self::preg_match('#^(?P<key>' . \Symfony\Component\Yaml\Inline::REGEX_QUOTED_STRING . '|[^ \'"\\{\\[].*?) *\\:(\\s+(?P<value>.+?))?\\s*$#u', $this->trimTag($values['value']), $matches))) {
            ...
            } elseif (self::preg_match('#^(?P<key>(?:![^\\s]++\\s++)?(?:' . \Symfony\Component\Yaml\Inline::REGEX_QUOTED_STRING . '|(?:!?!php/const:)?[^ \'"\\[\\{!].*?)) *\\:(\\s++(?P<value>.+))?$#u', \rtrim($this->currentLine), $values) && (\false === \strpos($values['key'], ' #') || \in_array($values['key'][0], ['"', "'"]))) {
                ...
                try {
                    $i = 0;
                    $evaluateKey = !(\Symfony\Component\Yaml\Yaml::PARSE_KEYS_AS_STRINGS & $flags);
                    // constants in key will be evaluated anyway
                    if (isset($values['key'][0]) && '!' === $values['key'][0] && \Symfony\Component\Yaml\Yaml::PARSE_CONSTANT & $flags) {
                        $evaluateKey = \true;
                    }
                    $key = \Symfony\Component\Yaml\Inline::parseScalar($values['key'], 0, null, $i, $evaluateKey);
                ...
                if ('<<' === $key && (!isset($values['value']) || !self::preg_match('#^&(?P<ref>[^ ]+)#u', $values['value'], $refMatches))) {
                    ...
                    if (isset($values['value'][0]) && '*' === $values['value'][0]) {
                        ...
                        if (\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $refValue instanceof \stdClass) {
                        ...
                        if (\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $parsed instanceof \stdClass) {
                            $parsed = (array) $parsed;
                        }
                        if (!\is_array($parsed)) {
                            throw new ParseException('YAML merge keys used with a scalar value instead of an array.', $this->getRealCurrentLineNb() + 1, $this->currentLine, $this->filename);
                        ...
                        if (isset($parsed[0])) {
                            ...
                            foreach ($parsed as $parsedItem) {
                                if (\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $parsedItem instanceof \stdClass) {
                ...
                if ($mergeNode) {
                    ...
                    if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
                        ...
                        if ('<<' === $key) {
                            ...
                            if (\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && $value instanceof \stdClass) {
                                $value = (array) $value;
                            }
                            $data += $value;
                ...
                if (\is_string($value) && $this->lines[0] === \trim($value)) {
                    try {
                        $value = \Symfony\Component\Yaml\Inline::parse($this->lines[0], $flags, $this->refs);
                    } catch (ParseException $e) {
                        $e->setParsedLine($this->getRealCurrentLineNb() + 1);
                        $e->setSnippet($this->currentLine);
                        throw $e;
                ...
                if (0 === $this->currentLineNb) {
                    ...
                    try {
                        return \Symfony\Component\Yaml\Inline::parse(\trim($value));
                    } catch (ParseException $e) {
        ...
        if (\Symfony\Component\Yaml\Yaml::PARSE_OBJECT_FOR_MAP & $flags && !\is_object($data) && 'mapping' === $context) {
            $object = new \stdClass();
    ...
    private function parseValue($value, $flags, $context)
    {
        ...
        if (self::preg_match('/^(?:' . self::TAG_PATTERN . ' +)?' . self::BLOCK_SCALAR_HEADER_PATTERN . '$/', $value, $matches)) {
            ...
            if ('' !== $matches['tag']) {
                if ('!!binary' === $matches['tag']) {
                    return \Symfony\Component\Yaml\Inline::evaluateBinaryScalar($data);
        ...
        try {
            ...
            if (null !== $quotation && self::preg_match('/^' . $quotation . '.*' . $quotation . '(\\s*#.*)?$/', $value)) {
                return \Symfony\Component\Yaml\Inline::parse($value, $flags, $this->refs);
            }
            $lines = [];
            while ($this->moveToNextLine()) {
            ...
            \Symfony\Component\Yaml\Inline::$parsedLineNumber = $this->getRealCurrentLineNb();
            $parsedValue = \Symfony\Component\Yaml\Inline::parse($value, $flags, $this->refs);
