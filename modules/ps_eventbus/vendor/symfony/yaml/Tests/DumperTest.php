...
EOF;
    ...
    {
        $dump = $this->dumper->dump(['foo' => new \Symfony\Component\Yaml\Tests\A(), 'bar' => 1], 0, 0, Yaml::DUMP_OBJECT);
    ...
    {
        $dump = $this->dumper->dump(['foo' => new \Symfony\Component\Yaml\Tests\A(), 'bar' => 1], 0, 0, \false, \true);
        $this->assertEquals('{ foo: !php/object \'O:30:"Symfony\\Component\\Yaml\\Tests\\A":1:{s:1:"a";s:3:"foo";}\', bar: 1 }', $dump, '->dump() is able to dump objects');
    }
    public function testObjectSupportDisabledButNoExceptions()
    {
        $dump = $this->dumper->dump(['foo' => new \Symfony\Component\Yaml\Tests\A(), 'bar' => 1]);
        $this->assertEquals('{ foo: null, bar: 1 }', $dump, '->dump() does not dump objects when disabled');
    }
    public function testObjectSupportDisabledWithExceptions()
    {
        $this->expectException('Symfony\\Component\\Yaml\\Exception\\DumpException');
        $this->dumper->dump(['foo' => new \Symfony\Component\Yaml\Tests\A(), 'bar' => 1], 0, 0, Yaml::DUMP_EXCEPTION_ON_INVALID_TYPE);
    }
    /**
     * @group legacy
     */
    ...
    {
        $this->expectException('Symfony\\Component\\Yaml\\Exception\\DumpException');
        $this->dumper->dump(['foo' => new \Symfony\Component\Yaml\Tests\A(), 'bar' => 1], 0, 0, \true);
    }
    ...
    {
        return ['empty string' => ['', "''"], 'null' => ["\x00", '"\\0"'], 'bell' => ["\x07", '"\\a"'], 'backspace' => ["\x08", '"\\b"'], 'horizontal-tab' => ["\t", '"\\t"'], 'line-feed' => ["\n", '"\\n"'], 'vertical-tab' => ["\v", '"\\v"'], 'form-feed' => ["\f", '"\\f"'], 'carriage-return' => ["\r", '"\\r"'], 'escape' => ["\x1b", '"\\e"'], 'space' => [' ', "' '"], 'double-quote' => ['"', "'\"'"], 'slash' => ['/', '/'], 'backslash' => ['\\', '\\'], 'del' => ["", '"\\x7f"'], 'next-line' => ["", '"\\N"'], 'non-breaking-space' => [" ", '"\\_"'], 'line-separator' => [" ", '"\\L"'], 'paragraph-separator' => [" ", '"\\P"'], 'colon' => [':', "':'"]];
    }
    ...
    public function testNonUtf8DataIsDumpedBase64Encoded()
    {
        ...
        $this->assertSame('!!binary ZsM/cg==', $this->dumper->dump("f\xc3?r"));
    }
    /**
    ...
    public function objectAsMapProvider()
    {
        ...
        $a = new \Symfony\Component\Yaml\Tests\A();
