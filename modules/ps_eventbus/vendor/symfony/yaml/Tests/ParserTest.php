...
class ParserTest extends TestCase
{
        ...
        if ($deprecated) {
            \set_error_handler(function ($type, $msg) use(&$deprecations) {
                if (\E_USER_DEPRECATED !== $type) {
                    ...
                    return \call_user_func_array('ps_eventbus_v3_0_7\\PHPUnit\\Util\\ErrorHandler::handleError', \func_get_args());
                }
                $deprecations[] = $msg;
                return null;
            });
    ...
    {
        $this->assertSame($expected, \var_export($this->parser->parse($yaml, Yaml::PARSE_KEYS_AS_STRINGS), \true), $comment);
    }
...
bar: 1
EOF;
        $this->assertEquals(['foo' => new \Symfony\Component\Yaml\Tests\B(), 'bar' => 1], $this->parser->parse($input, Yaml::PARSE_OBJECT), '->parse() is able to parse objects');
    }
    /**
     * @group legacy
     */
...
bar: 1
EOF;
        $this->assertEquals(['foo' => new \Symfony\Component\Yaml\Tests\B(), 'bar' => 1], $this->parser->parse($input, \false, \true), '->parse() is able to parse objects');
    }
    /**
     * @group legacy
    ...
    {
        $this->assertEquals(['foo' => new \Symfony\Component\Yaml\Tests\B(), 'bar' => 1], $this->parser->parse($yaml, Yaml::PARSE_OBJECT), '->parse() is able to parse objects');
    }
...
map2:
  - !php/const 'Symfony\\Component\\Yaml\\Tests\\B::FOO': 'value_0'
    bar: 'value_1'
YAML;
        $this->assertSame(['map1' => [['foo' => 'value_0', 'bar' => 'value_1']], 'map2' => [['foo' => 'value_0', 'bar' => 'value_1']]], $this->parser->parse($yaml, Yaml::PARSE_CONSTANT));
    }
