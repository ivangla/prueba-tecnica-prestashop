 ...
 */
class ReflectionClassResource implements \Symfony\Component\Config\Resource\SelfCheckingResourceInterface, \Serializable
{
    private $files = [];
    private $className;
    ...
    private function generateSignature(\ReflectionClass $class)
    {
        ...
        if (\defined('HHVM_VERSION')) {
            foreach ($class->getMethods(\ReflectionMethod::IS_PUBLIC | \ReflectionMethod::IS_PROTECTED) as $m) {
                ...
                (yield \preg_replace('/^  @@.*/m', '', new \Symfony\Component\Config\Resource\ReflectionMethodHhvmWrapper($m->class, $m->name)));
            }
        } else {
            foreach ($class->getMethods(\ReflectionMethod::IS_PUBLIC | \ReflectionMethod::IS_PROTECTED) as $m) {
                $defaults = [];
...
class ReflectionMethodHhvmWrapper extends \ReflectionMethod
{
    public function getParameters()
    {
        ...
        foreach (parent::getParameters() as $i => $p) {
            $params[] = new \Symfony\Component\Config\Resource\ReflectionParameterHhvmWrapper([$this->class, $this->name], $i);
        }
        return $params;
    }
}
