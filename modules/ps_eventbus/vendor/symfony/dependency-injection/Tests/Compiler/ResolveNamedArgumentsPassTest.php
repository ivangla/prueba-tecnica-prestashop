...
class ResolveNamedArgumentsPassTest extends TestCase
{
    ...
    {
        $container = new ContainerBuilder();
        $container->register('factory', \Symfony\Component\DependencyInjection\Tests\Compiler\NoConstructor::class);
        $definition = $container->register('foo', \Symfony\Component\DependencyInjection\Tests\Compiler\NoConstructor::class)->setFactory([new Reference('factory'), 'create'])->setArguments(['$apiKey' => '123']);
        $pass = new ResolveNamedArgumentsPass();
        $pass->process($container);
    ...
    public function testClassNotExist()
    {
        $this->expectException('Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException');
        $container = new ContainerBuilder();
        $definition = $container->register(\Symfony\Component\DependencyInjection\Tests\Compiler\NotExist::class, \Symfony\Component\DependencyInjection\Tests\Compiler\NotExist::class);
        $definition->setArguments(['$apiKey' => '123']);
        $pass = new ResolveNamedArgumentsPass();
        $pass->process($container);
    }
    public function testClassNoConstructor()
    {
        $this->expectException('Symfony\\Component\\DependencyInjection\\Exception\\RuntimeException');
        $container = new ContainerBuilder();
        $definition = $container->register(\Symfony\Component\DependencyInjection\Tests\Compiler\NoConstructor::class, \Symfony\Component\DependencyInjection\Tests\Compiler\NoConstructor::class);
        $definition->setArguments(['$apiKey' => '123']);
        $pass = new ResolveNamedArgumentsPass();
        $pass->process($container);
    }
