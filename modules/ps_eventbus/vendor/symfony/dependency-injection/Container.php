 ...
 */
class Container implements \Symfony\Component\DependencyInjection\ResettableContainerInterface
{
    ...
    public function has($id)
    {
        for ($i = 2;;) {
            ...
            if (!$this->methodMap && !$this instanceof \Symfony\Component\DependencyInjection\ContainerBuilder && __CLASS__ !== static::class && \method_exists($this, 'get' . \strtr($id, $this->underscoreMap) . 'Service')) {
        ...
        for ($i = 2;;) {
            ...
            try {
                if (isset($this->fileMap[$id])) {
                    ...
                    $id = $normalizedId;
                    continue;
                } elseif (!$this->methodMap && !$this instanceof \Symfony\Component\DependencyInjection\ContainerBuilder && __CLASS__ !== static::class && \method_exists($this, $method = 'get' . \strtr($id, $this->underscoreMap) . 'Service')) {
                    // We only check the convention-based factory in a compiled container (i.e. a child class other than a ContainerBuilder,
                    // and only when the dumper has not generated the method map (otherwise the method map is considered to be fully populated by the dumper)
                    @\trigger_error('Generating a dumped container without populating the method map is deprecated since Symfony 3.2 and will be unsupported in 4.0. Update your dumper to generate the method map.', \E_USER_DEPRECATED);
    ...
    {
        $ids = [];
        if (!$this->methodMap && !$this instanceof \Symfony\Component\DependencyInjection\ContainerBuilder && __CLASS__ !== static::class) {
            // We only check the convention-based factory in a compiled container (i.e. a child class other than a ContainerBuilder,
            // and only when the dumper has not generated the method map (otherwise the method map is considered to be fully populated by the dumper)
            @\trigger_error('Generating a dumped container without populating the method map is deprecated since Symfony 3.2 and will be unsupported in 4.0. Update your dumper to generate the method map.', \E_USER_DEPRECATED);
    ...
    protected function getEnv($name)
    {
        ...
        if (!$this->has($id = 'container.env_var_processors_locator')) {
            $this->set($id, new \Symfony\Component\DependencyInjection\ServiceLocator([]));
        }
        if (!$this->getEnv) {
        ...
        $processor = $processors->has($prefix) ? $processors->get($prefix) : new \Symfony\Component\DependencyInjection\EnvVarProcessor($this);
