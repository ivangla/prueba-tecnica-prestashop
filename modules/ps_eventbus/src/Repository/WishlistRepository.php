...
class WishlistRepository
{
    ...
    public function getBaseQuery()
    {
        ...
        $query = new \DbQuery();
        $query->from('wishlist', 'w')->where('w.id_shop = ' . $shopId);
        return $query;
    }
    /**
     * @param int $offset
    ...
    {
        $query = $this->getBaseQuery()->select('(COUNT(w.id_wishlist) - ' . (int) $offset . ') as count');
    ...
    public function getWishlistsIncremental($limit, $wishlistIds)
    {
        $query = $this->getBaseQuery();
        $this->addSelectParameters($query);
        $query->where('w.id_wishlist IN(' . \implode(',', \array_map('intval', $wishlistIds)) . ')')->limit($limit);
        return $this->db->executeS($query);
    }
    /**
     * @param int $offset
