...
class ImageTypeRepository
{
    ...
    {
        $query = $this->getBaseQuery()->select('(COUNT(it.id_image_type) - ' . (int) $offset . ') as count');
    ...
    public function getImageTypesIncremental($limit, $imageTypeIds)
    {
        $query = $this->getBaseQuery();
        $this->addSelectParameters($query);
        $query->where('it.id_image_type IN(' . \implode(',', \array_map('intval', $imageTypeIds)) . ')')->limit($limit);
        return $this->db->executeS($query);
    }
    /**
     * @param int $offset
