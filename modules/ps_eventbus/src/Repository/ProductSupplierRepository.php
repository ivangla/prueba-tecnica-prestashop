...
class ProductSupplierRepository
{
    ...
    {
        $query = $this->getBaseQuery()->select('(COUNT(ps.id_product_supplier) - ' . (int) $offset . ') as count');
    ...
    public function getProductSuppliersIncremental($limit, $productIds)
    {
        $query = $this->getBaseQuery();
        $this->addSelectParameters($query);
        $query->where('ps.id_product IN(' . \implode(',', \array_map('intval', $productIds)) . ')')->limit($limit);
        return $this->db->executeS($query);
    }
    /**
     * @param int $offset
