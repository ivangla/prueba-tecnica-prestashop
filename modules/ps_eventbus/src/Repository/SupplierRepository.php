...
class SupplierRepository
{
    ...
    {
        $query = $this->getBaseQuery($langIso)->select('(COUNT(su.id_supplier) - ' . (int) $offset . ') as count');
    ...
    public function getSuppliersIncremental($limit, $langIso, $supplierIds)
    {
        $query = $this->getBaseQuery($langIso);
        $this->addSelectParameters($query);
        $query->where('su.id_supplier IN(' . \implode(',', \array_map('intval', $supplierIds)) . ')')->limit($limit);
        return $this->db->executeS($query);
    }
    /**
     * @param string $langIso
    ...
    public function getBaseQuery($langIso)
    {
        ...
        $query = new \DbQuery();
        $query->from('supplier', 'su')->innerJoin('supplier_lang', 'sul', 'su.id_supplier = sul.id_supplier AND sul.id_lang = ' . (int) $langId)->innerJoin('supplier_shop', 'sus', 'su.id_supplier = sus.id_supplier AND sus.id_shop = ' . $shopId);
        return $query;
    }
    /**
     * @param int $offset
