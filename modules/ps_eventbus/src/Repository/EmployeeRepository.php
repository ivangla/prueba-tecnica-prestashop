...
class EmployeeRepository
{
    ...
    private function getBaseQuery()
    {
        ...
        $query = new \DbQuery();
        $query->from('employee', 'e')->leftJoin('employee_shop', 'es', 'es.id_employee = e.id_employee');
    ...
    {
        $query = $this->getBaseQuery()->select('(COUNT(e.id_employee) - ' . (int) $offset . ') as count');
    ...
    public function getEmployeesIncremental($limit, $employeeIds)
    {
        $query = $this->getBaseQuery();
        $this->addSelectParameters($query);
        $query->where('e.id_employee IN(' . \implode(',', \array_map('intval', $employeeIds)) . ')')->limit($limit);
        return $this->db->executeS($query);
    }
    /**
     * @param int $offset
