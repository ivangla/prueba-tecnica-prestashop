...
class StockRepository
{
    ...
    public function getBaseQuery()
    {
        ...
        $query = new \DbQuery();
        $query->from('stock_available', 'sa')->where('sa.id_shop = ' . (int) $shopId);
        return $query;
    }
    /**
     * @param int $offset
    ...
    {
        $query = $this->getBaseQuery()->select('(COUNT(sa.id_stock_available) - ' . (int) $offset . ') as count');
    ...
    public function getStocksIncremental($limit, $productIds)
    {
        $query = $this->getBaseQuery();
        $this->addSelectParameters($query);
        $query->where('sa.id_product IN(' . \implode(',', \array_map('intval', $productIds)) . ')')->limit($limit);
        return $this->db->executeS($query);
    }
    /**
     * @param int $offset
