...
class TaxRepository
{
    ...
    private function getBaseQuery()
    {
        ...
        $query = new \DbQuery();
        $query->from('tax', 't')->innerJoin('tax_rule', 'tr', 'tr.id_tax = t.id_tax')->innerJoin('tax_rules_group', 'trg', 'trg.id_tax_rules_group = tr.id_tax_rules_group')->innerJoin('tax_rules_group_shop', 'trgs', 'trgs.id_tax_rules_group = tr.id_tax_rules_group')->innerJoin('tax_lang', 'tl', 'tl.id_tax = t.id_tax')->where('trgs.id_shop = ' . $shopId)->where('tl.id_lang = ' . $language);
        return $query;
    }
    /**
     * @param int $zoneId
     ...
     */
    public function getCarrierTaxesByZone($zoneId, $taxRulesGroupId, $active = \true)
    {
