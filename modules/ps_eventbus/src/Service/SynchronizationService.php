...
class SynchronizationService
{
    ...
    public function __construct(EventbusSyncRepository $eventbusSyncRepository, IncrementalSyncRepository $incrementalSyncRepository, LiveSyncRepository $liveSyncRepository, \PrestaShop\Module\PsEventbus\Service\ProxyServiceInterface $proxyService, PayloadDecorator $payloadDecorator)
    {
    ...
    public function handleIncrementalSync(PaginatedApiDataProviderInterface $dataProvider, $type, $jobId, $limit, $langIso, $scriptStartTime, bool $isFull)
    {
        ...
        if (empty($objectIds)) {
            return ['total_objects' => 0, 'has_remaining_objects' => \false, 'remaining_objects' => 0];
        }
    ...
    {
        return \array_merge(['total_objects' => \count($data), 'has_remaining_objects' => $remainingObjects > 0, 'remaining_objects' => $remainingObjects, 'md5' => $this->getPayloadMd5($data)], $syncResponse);
    }
    /**
     * @param array $payload
     *
    ...
    {
        return \md5(\implode(' ', \array_map(function ($payloadItem) {
