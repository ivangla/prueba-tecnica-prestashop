...
class SpecificPriceService
{
     ...
     * @@throws \PrestaShopException
     */
    private function getPriceStatic($id_product, $id_product_attribute, $specificPriceId, $usetax = \true, $usereduc = \true, \Context $context = null, $decimals = 6, $divisor = null, $only_reduc = \false, $id_customer = null, $id_cart = null, $id_address = null, &$specific_price_output = null, $use_group_reduction = \true)
    {
        if (!$context) {
            /** @var \Context $context */
        ...
        $address = \Address::initialize($id_address, \true);
        ...
        if ($usetax != \false && !empty($address->vat_number) && $address->id_country != \Configuration::get('VATNUMBER_COUNTRY') && \Configuration::get('VATNUMBER_MANAGEMENT')) {
            $usetax = \false;
        }
        ...
        return $this->priceCalculation($shopId, $id_product, $id_product_attribute, $specificPriceId, $id_country, $id_state, $zipcode, $id_currency, $id_group, $usetax, $decimals, $only_reduc, $usereduc, $specific_price_output, $use_group_reduction);
    }
    /**
     * @param int $id_shop
     * @param int $id_product
     ...
     */
    private function priceCalculation($id_shop, $id_product, $id_product_attribute, $specificPriceId, $id_country, $id_state, $zipcode, $id_currency, $id_group, $use_tax, $decimals, $only_reduc, $use_reduc, &$specific_price, $use_group_reduction, $id_customization = 0)
    {
        static $address = null;
        static $context = null;
        if ($context == null) {
        ...
        if (!isset(self::$_pricesLevel2[$cache_id_2])) {
            ...
            if (\is_array($res) && \count($res)) {
                foreach ($res as $row) {
                    $array_tmp = ['price' => $row['price'], 'ecotax' => $row['ecotax'], 'attribute_price' => isset($row['attribute_price']) ? $row['attribute_price'] : null];
