...
class DeletedObjectsService
{
    ...
    public function __construct(\Context $context, DeletedObjectsRepository $deletedObjectsRepository, \PrestaShop\Module\PsEventbus\Service\ProxyServiceInterface $proxyService)
    {
    ...
    public function handleDeletedObjectsSync($jobId, $scriptStartTime)
    {
        ...
        if (empty($deletedObjects)) {
            return ['job_id' => $jobId, 'total_objects' => 0, 'syncType' => 'full'];
        }
        $data = $this->formatData($deletedObjects);
        $response = $this->proxyService->delete($jobId, $data, $scriptStartTime);
        if ($response['httpCode'] == 200) {
            foreach ($data as $dataItem) {
                $this->deletedObjectsRepository->removeDeletedObjects($dataItem['collection'], $dataItem['deleteIds'], $shopId);
            }
        }
        return \array_merge(['job_id' => $jobId, 'total_objects' => \count($data), 'syncType' => 'full'], $response);
    }
    /**
     * @param array $data
     *
    ...
    private function formatData(array $data)
    {
        return \array_map(function ($dataItem) {
            return ['collection' => $dataItem['type'], 'deleteIds' => \explode(';', $dataItem['ids'])];
        }, $data);
    }
}
