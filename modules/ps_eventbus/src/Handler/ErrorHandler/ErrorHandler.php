 ...
 * Handle Error.
 */
class ErrorHandler implements \PrestaShop\Module\PsEventbus\Handler\ErrorHandler\ErrorHandlerInterface
{
    /**
     * @var ?\Raven_Client
     */
     ...
     * @@throws Exception
     */
    public function handle($error, $code = null, $throw = \true, $data = null)
    {
        if (!$this->client) {
            return;
        }
        $this->client->captureException($error, $data);
        if (\is_int($code) && \true === $throw) {
